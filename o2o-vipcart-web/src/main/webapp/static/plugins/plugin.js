...
(function ($ns) {
    ...
    $ns.open = function (paramsObj) {
        ...
        paramsObj.height ? $id.height(paramsObj.height) : "";
        $ns.setLayout($id, top);
    };
    ...
    function buildCommonContent(paramsObj) {
        ...
        if (commonType == 1) {
            commonContent = "<p><input type='button' id='" + confirmId + "' class='b_foot_long' value='确 定'/><input type='button' id='" + cancelId + "' class='b_foot_long'' value='取 消'/></p>";
        } else if (commonType == 2) {
            commonContent = "<p><input type='button' id='" + cancelId + "' class='b_foot_long' value='关 闭'/></p>";
        }
        var $id = $("<div id='" + id + "' class='ajax_window'>" +
            "<div class='ajax_wait'>" +
            commonContent +
            "</div>" +
            "</div>");
        ...
        $id.find("input[id$='" + id + "']").click(function () {
    ...
    $ns.getClientHeight = function () {
        ...
        if (bodyHeight && documentHeight) {
            //clientHeight = (bodyHeight< documentHeight)?bodyHeight:documentHeight;
            clientHeight = Math.min(bodyHeight, documentHeight);
        } else {
            //clientHeight = (bodyHeight> documentHeight)?bodyHeight:documentHeight;
    ...
    $ns.tableListStyle = function () {
        ...
        $(".list tr:gt(0)").each(function () {
            ...
            var clickColor = "tr_blue";
            var $clickColor = $("." + clickColor);//上一次点击行
            $clickColor.removeClass().addClass($clickColor.attr("oldClass"));
            $(this).attr("oldClass", $(this).attr("class"));
            $(this).removeClass().addClass(clickColor);
        });
    };
     ...
     * @param url
     */
    $ns.refreshPageAjax = function (pageId, url, param) {
        var $pageId = paramJquery(pageId);
