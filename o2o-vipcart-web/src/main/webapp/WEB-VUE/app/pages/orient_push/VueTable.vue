    ...
    <table v-if="tables.length > 0" class="ui radio table mini">
      ...
      <tbody>
        <tr v-for="item in tables">
          <td><vue-tip :text="item.activityCode" width="200"></vue-tip></td>
          <td>{{item.activityName}}</td>
          <td>{{item.batchsNo}}</td>
          <td>{{item.createTime}}</td>
          <td>{{item.createPin}}
          <td>{{item.pushState | enum 'pushState'}}</td>
          <td>
            <div class="ui teal buttons">
              <a class="ui button" href="vipcart.html?activityCode={{item.activityCode}}&activityName={{item.activityName}}&activityState=1&pageType=orient_push">管理优惠券</a>
            </div>
          </td>
        </tr>
      </tbody>
...
export default {
  ...
  data() {
    return {
			table: {
				ajaxURL: '/activity/pageQueryDirectionActivityInfo'
			},
      tables: [],
      checkList: [],
      query: {
