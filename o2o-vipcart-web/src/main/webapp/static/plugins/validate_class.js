 ...
 * @param elem
 */
function tipsInit(elem) {
    var value = $(elem).val();
    var tips = $(elem).attr("tips");
    if (tips) {
        if (!value) {
            $(elem).val(tips).css("color", "#DFDFDF");
        }
 ...
 * @param elem
 */
function clearTips(elem) {
    var tips = elem.getAttribute("tips");
    if (tips && tips == elem.value) {
        elem.value = "";
    }
}
/**
