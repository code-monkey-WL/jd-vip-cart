import '../../../css/edit_city_push.less';
  ...
  methods: {
    ...
    validate() {
      ...
      let fields = {
        ...
        totalNum: {identifier : 'totalNum', rules: [{type:'empty', prompt:'请输入生成码数量'},{type:'integer[1..99]', prompt:'格式错误'}]},
        codeBeginTime: {identifier : ['codeBeginTime', 'codeEndTime'], rules: [{type:'empty', prompt:'请选择码有效期', parents: '.pfield'}]},
        canUseTimes: {identifier : 'canUseTimes', rules: [{type:'empty', prompt:'请输入生成码数量'},{type:'integer[1..1000000]', prompt:'格式错误'}]}
      };
    ...
    onSave() {
      ...
      $.ajax({
        ...
        success: (result) => {
          ...
          if(result.code == '0') {
            alert("提示：创建成功！");
            window.location = "edit_vipcart.html?type=1&pageType=city_push&activityCode=" + result.result;
          } else {
