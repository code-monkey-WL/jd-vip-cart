package com.jd.o2o.vipcart.service.base.impl;

import com.jd.o2o.vipcart.common.dao.BaseDao;
import com.jd.o2o.vipcart.common.service.AbstractBaseService;
import com.jd.o2o.vipcart.dao.UserInfoDao;
import com.jd.o2o.vipcart.domain.entity.UserInfoEntity;
import com.jd.o2o.vipcart.service.base.UserInfoService;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.stereotype.Service;
import javax.annotation.Resource;

@Service
public class UserInfoServiceImpl extends AbstractBaseService<UserInfoEntity,Long> implements UserInfoService {
    private static final Logger LOGGER = LoggerFactory.getLogger(UserInfoServiceImpl.class);
    @Resource
    private UserInfoDao userInfoDao;

    @Override
    protected BaseDao<UserInfoEntity, Long> getBaseDao() {
        return userInfoDao;
    }
}
