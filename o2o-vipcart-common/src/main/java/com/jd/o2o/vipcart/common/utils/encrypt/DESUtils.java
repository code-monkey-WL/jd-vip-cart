...
public class DESUtils {
	...
	public byte[] decode(String s) {
		return decode(s.getBytes(), getKey());
	}
	 ...
	 * @return	明文字节。
	 */
	public byte[] decode(byte[] sBytes) {
		return decode(sBytes, getKey());
	}
	 ...
	 * @return
	 */
	public byte[] decode(String s, long key) {
		return decode(s.getBytes(), key);
	}
	 ...
	 * @return	byte[]，明文字节。
	 */
	public byte[] decode(byte[] sBytes, long key) {
		DES des = new DES(key);
		...
		for (int i = 0; i < (sBytes.length / 16); i++) {
			...
			DES.long2bytes(des.des(x, false), tmpResult);
			for (int k = 0; k < 8; k++) {
	...
	public byte[] encode(String s) {
		return encode(s.getBytes(), getKey());
	}
	 ...
	 * @return	密文字节。
	 */
	public byte[] encode(byte[] sBytes) {
		return encode(sBytes, getKey());
	}
	 ...
	 * @return	密文字节。
	 */
	public byte[] encode(String s, long key) {
		return encode(s.getBytes(), key);
	}
	
	/**
	 * 以指定的密钥，加密明文字节。
	 */
	public byte[] encode(byte[] sBytes, long key) {
		DES des = new DES(key);
		byte space = 0x20;
		...
		for (int i = 0; i < (newLength / 8); i++) {
			...
			DES.long2bytes(des.des(x, true), result);
  ...
  public static void main(String[] args) {
    String str = "123456";
    byte[] encodeBuff = DESUtils.instance.encode(str, 12345678);
    System.out.println(new String(encodeBuff));
  }
    ...
    static class DES {
        ...
        public DES(String key) {
            ...
            keySchedule(DES.bytes2long(bKey));
        }
